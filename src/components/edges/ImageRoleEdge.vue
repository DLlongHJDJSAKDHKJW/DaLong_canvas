<template>
  ...
  <EdgeLabelRenderer>
    ...
    >
      ...
      >
        <button 
          class="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-full bg-[var(--bg-secondary)] border border-[var(--border-color)] text-[var(--text-primary)] shadow-sm hover:shadow transition-shadow"
        >
          {{ currentRoleLabel }}
...
const imageRoleOptions = [
  { label: '底图 (背景/结构)', key: 'base_image' },
  { label: '角色参考', key: 'character_ref' },
  { label: '风格参考', key: 'style_ref' },
  { label: '首帧 (视频)', key: 'first_frame_image' },
  { label: '尾帧 (视频)', key: 'last_frame_image' },
  { label: '参考图', key: 'input_reference' }
]

// Current role from edge data | 从边数据获取当前角色
const currentRole = computed(() => props.data?.imageRole || 'base_image')

// Current role label | 当前角色标签
const currentRoleLabel = computed(() => {
  const option = imageRoleOptions.find(o => o.key === currentRole.value)
  return option?.label || '底图'
})
...
const handleRoleSelect = (role) => {
  // If selecting first_frame or last_frame, ensure uniqueness
