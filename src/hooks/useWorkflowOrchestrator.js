...
export const useWorkflowOrchestrator = () => {
  ...
  const analyzeIntent = async (userInput) => {
    ...
    try {
      ...
      for await (const chunk of streamChatCompletions({
        model: 'gpt-5.4',
        messages: [
          { role: 'system', content: [{ type: 'text', text: INTENT_ANALYSIS_PROMPT }] },
          { role: 'user', content: [{ type: 'text', text: userInput }] }
        ]
      })) {
        response += (typeof chunk === 'object' ? (chunk.content || '') : chunk)
      }
  ...
  const executeTextToImage = async (imagePrompt, position) => {
    ...
    const imageNode = await waitForOutputReady(imageNodeId)
