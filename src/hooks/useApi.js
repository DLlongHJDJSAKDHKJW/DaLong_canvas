...
export const useChat = (options = {}) => {
  ...
  const send = async (content, stream = true, chatOptions = {}) => {
    ...
    try {
      ...
      if (images.length > 0) {
        ...
        // 兜底：如果内容为空，确保不发送空请求
        userContent = content || 'Please optimize the prompt.'
      }

      const finalSystemPrompt = chatOptions.systemPrompt || options.systemPrompt
      const msgList = [
      ...
      const adaptedParams = adaptRequest('chat', {
        model: chatOptions.model || options.model || 'gemini-3.1-pro-preview',
        messages: msgList,
        stream: stream, // 确保传递流式标志
        temperature: chatOptions.temperature ?? options.temperature ?? 0.7,
        max_completion_tokens: chatOptions.maxTokens ?? options.maxTokens ?? 1024
      })
      ...
      if (stream) {
        ...
        for await (const chunk of streamChatCompletions(
          ...
          if (chunk.reasoning) {
            ...
            currentReasoning.value = fullReasoning
          }
          if (chunk.content) {
          ...
          // 回调 UI 更新 | Callback UI update
          if (chatOptions.onStream) {
        ...
        if (!fullResponse.trim()) {
          const fallbackResponse = await chatCompletions({
            ...adaptedParams,
            stream: false
          })
          fullResponse = adaptResponse('chat', fallbackResponse) || ''
          currentResponse.value = fullResponse
        }
      ...
      const finalResponse = adaptResponse('chat', response) || ''
      messages.value.push({ role: 'user', content })
      messages.value.push({ role: 'assistant', content: finalResponse })
      currentResponse.value = finalResponse
      setSuccess()
  ...
  return { loading, error, status, messages, currentResponse, currentReasoning, send, stop, clear, reset }
}
...
export const useImageGeneration = () => {
  ...
  const normalizeImageError = (err) => {
    const responseData = err?.response?.data || {}
    const responseStatus = err?.response?.status
    const apiStatus = responseData?.status
    const apiReason = responseData?.error?.status || responseData?.error?.reason || responseData?.reason
    const apiMessage = responseData?.error?.message || responseData?.message || err?.message || ''
    const rawText = [responseStatus, apiStatus, apiReason, apiMessage]
      .filter(Boolean)
      .join(' ')
      .toUpperCase()
    ...
    if (
      ...
      apiReason === 'RESOURCE_EXHAUSTED' ||
      rawText.includes('RESOURCE_EXHAUSTED') ||
      rawText.includes('QUOTA') ||
      rawText.includes('RATE LIMIT')
    ) {
      return new Error('图片生成失败：当前图像生成额度不足，或通道正被限流，请稍后重试或更换模型/API Key。')
    }

    if (apiMessage) {
      return new Error(`图片生成失败：${apiMessage}`)
    }

    return err instanceof Error ? err : new Error('图片生成失败，请稍后重试。')
  }
  ...
  const generate = async (params) => {
    ...
    try {
      ...
      // 统一解析响应（Gemini inlineData 或 OpenAI imageUrl）
      ...
      for (const candidate of candidates) {
        for (const part of (candidate?.content?.parts || [])) {
          if (part.inlineData?.data) {
            ...
            adaptedData.push({ url: part.imageUrl, revisedPrompt: '' })
          }
        }
      }

      console.log('[Image Generation] Got images:', adaptedData.length)
      ...
      console.error('❌ [Image Generation] Error response:', err.response?.data)
      console.error('❌ [Image Generation] Error status:', err.response?.status)
      console.error('❌ [Image Generation] Error message:', err.message)
      const normalizedError = normalizeImageError(err)
